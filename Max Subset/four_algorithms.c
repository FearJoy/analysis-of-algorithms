...
{
    // Check that:
    //      1. the arg count is exactly 4 or 5
    //      2. the provided array size is positive
    //      3. the algorithm selector is 1, 2, 3, or 4
    if (argc < 4 || argc > 5 || stringToInt(argv[1]) < 0 || stringToInt(argv[3]) < 1 || stringToInt(argv[3]) > 4) {
        printf("Usage - %s input_size_in_thousands input_file algorithm_switch [debug_switch]\n", argv[0]);
        printf("\tinput_size_in_thousands - provide a positive integer that reflects (array_size)/1000\n");
        printf("\t\tUse 0 for files with arrays of various sizes\n");
        printf("\t\tExample: array of 100,000 integers, then use array_size_in_thousands = 100\n");
        printf("\tinput_file - name of the file to read from in current directory\n");
        printf("\talgorithm_switch - a number of 1, 2, 3, or 4. The algorithms are:\n");
        printf("\t                   (1) enumeration, (2) better enumeration, (3) divide & conquer, (4) linear time\n");
        printf("\tdebug_switch - optional, use 1 to enable debug output\n");
        exit(1);
    }

    int DEBUG = 0;

    // Check that 5 arguments were passed, then determine how the DEBUG flag is set
    if (argc == 5 && stringToInt(argv[4]) == 1)
        DEBUG = 1;

    if (DEBUG == 1) {
        printf("**** IN DEBUG OUTPUT MODE ***\n");
        printf("*** argc    (arg count) - %d\n", argc);
        printf("*** argv[0] (program name) - %s\n", argv[0]);
        printf("*** argv[1] (array size * 1,000) - %s\n", argv[1]);
        printf("*** argv[2] (input file) - %s\n", argv[2]);
        printf("*** argv[3] (algorithm selector [1,2,3,4]) - %s\n", argv[3]);
        printf("*** argv[4] (debug, [1]) - %s\n", argv[4]);
    }

    struct result* myResults = (struct result*)malloc(sizeof(struct result));
    assert(myResults != NULL);

    int arraySize = 1000 * stringToInt(argv[1]);
    char *inputFile = argv[2];
    int algorithmSelector = stringToInt(argv[3]);
    ...
    if (DEBUG == 1)
        printf("*** array size (1000 * %s, from argv[1]) - %d\n", argv[1], arraySize);
    ...
    int qtyInArray = readArray(fp, numberArray, &arraySize);
    int msec = 0;
    int lineNum = 1;

    do
    {
        printf("\n\nWriting computation time results for %s, line #%d, array size %d..\n", argv[2], lineNum, arraySize);
...
/*  Prints out the values from an array to a file on one line, followed by '\n'
    params:
